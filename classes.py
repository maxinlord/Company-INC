...
class DevSoftware:
    ...
    def get_quantity_device(self, device: int, lvl: int):
        try:
            return get_2dot_data(key=f'quantity_device_{device}', where='id_company', meaning=self.user.id, table='dev_software', where_data='lvl', meaning_data=str(lvl), get_data='quantity')
    ...
    def quantity_devices(self):
        ...
        y = True
        while y:
            data = parse_2dot_data(key=f'quantity_device_{ind}', where='id_company', meaning=self.user.id, table='dev_software')
            ind_q = data[0].index('quantity')
            for i in data[1:]:
                q_devices += i[ind_q]
            try:
                ind += 1
                self.user.BotDB.get(table='dev_software', key=f'quantity_device_{ind}', where='id_company', meaning=self.user.id)
            except:
                y = False
        return q_devices
    ...
    def data_centre_foreign(self):
        return get_2dot_data(key=f'data_centre', where='id_company', meaning=self.user.id, table='dev_software', meaning_data='1', get_data='foreign')
    
    @property
    def data_centre_home(self):
        return get_2dot_data(key=f'data_centre', where='id_company', meaning=self.user.id, table='dev_software', meaning_data='1', get_data='home')
